...
export function initSpeed(satellite, vec, speed) {
  satellite.V.add(vec.multiplyScalar(speed));
}
    ...
    if (satellites.length > 1) {
      for (let j = i; j < satellites.length; j++) {
        if (i != j) {
          ...
          distanceVector.subVectors(
            element.object.position,
            element2.object.position
          );
          const distance = distanceVector.lengthSq();
          if (distance < 70000000000) {
            let v = element.V.multiplyScalar(element.mass)
              .add(element2.V.multiplyScalar(element2.mass))
              .divideScalar(element.mass + element2.mass);
            let mTotal = element.mass + element2.mass;

            element.mass = mTotal;
            let index = satellites.indexOf(element2);
            satellites.splice(index, 1);
            ...
            destroyFolder(index);
            element.object.scale.multiplyScalar(2);
            element.V.copy(v);
          }
        }
      }
